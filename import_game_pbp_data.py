...
def get_games():
    ...
    The first element of each tuple is the week, followed by the home team name, then the away team, then the url to the PFR boxscore, to be used
    by get_game_data.
    """
...
def get_game_data(url):
    ...
    (row class, quarter, time remaining in quarter 'mm:ss', down, distance, side of field, yard line, description, away score, home score).
    Each element in the tuple is a string.
    """
    site = urllib.urlopen(url)
    sitetext = site.read()
    site.close()
    # I'll only search from where the play-by-play section of the site starts to the end.
    pbptext = sitetext.partition('Full Play-By-Play')[2]
    # The below regex is complicated, but it works. It's based on the fact that PFR stores its play-by-play data in an html table. If this code breaks in the future, it's
    # probably because PFR changed its table formatting. Each group in the regex is named in the block comment above. 
...
def get_off_play_data(home_team, away_team, game_data):
    """
    Given game_data in the form of the output of get_game_data, returns a tuple of two lists of lists, the first of which corresponds to the home team, and the 
    second of which corresponds to the away team. In each outer list is an inner list of data from offensive plays, including time remaining in the game in 
    seconds; the down; yards to go; field position indicated by a yardline between 0-100, with 0-50 indicating the play started on the offensive team's
    ...
    and positive if the offensive team is winning; and a 0 if the play was a run, and a 1 if the play was a 
    pass.
    ...
    """ 
    # Right now I'm only considering the 2015 season, but I want to allow for the study of other seasons as well, so I'll set a year.
    year = 2015
    ...
    # one if it's kicked from the away team's side of the field. Since the opening kickoff is always kicked from the kicking team's side of the field, this should always be
    # accurate. Note that the inital value for homeaway is not the one corresponding to the team who first possesses the ball on offense, this code will automatically switch
    # homeaway when it looks at the second play (the first play from scrimmage), which will have the "divider" tag. If the abbreviaton for the kickoff team does not match
    # either input team, it prints an error, but still initializes homeaway to 0. This error message will be useful if PFR ever changes its three-letter team name 
    # abbreviations.
    ...
    if team_dict[game_data[0][5]] == home_team:
        ...
        homeaway = 0
        print 'Error: starting team abbreviation in ' + away_team + ' @ ' + home_team + ' does not match home or away team.'
    ...
    # half is always the team who did not kick off the first half.
    i = 0
    home_data = []
    away_data = []
    ...
    # Here lies the primary while loop, which iterates over every element of game_data and 
    while i < len(game_data):
        # st is a flag indicating whether the current play is a special teams play. It is by default 0 for a play, but can get switched later on.
        st = 0
        ...
        if game_data[i][1] == '3' and game_data[i-1][1] == '2':
            ...
            # after the second-half kickoff, so I made it so that 'kicks off' in the previous play also indicates a change of possession.
            if ('divider' in game_data[i][0]) or ('kicks off' in game_data[i-1][7]):
                homeaway = (homeaway + 1) % 2
            # This line gets the time remaining in the game in seconds. From left to right, it's given by (quarters after current quarter)*900 + 60*(minutes left in current
            # quarter) + (seconds left in current quarter).
            ...
            if ('kick' in game_data[i][7].lower()) or ('punt' in game_data[i][7].lower()) or ('field goal' in game_data[i][7].lower()) or ('timeout' in game_data[i][7].lower()) or ('conversion' in game_data[i][7].lower()) or ('no play' in game_data[i][7].lower()) or (' -- ' == game_data[i][7].lower()):
                st = 1
            # I want to make sure I catch all non-offensive plays with the previous line. One way to check is to see if this play has a down and distance (elements 3 and 4 
            # of the gamedata[i] tuple) associated with it. If not, it's a special teams play.
            if (not st) and ((not game_data[i][3]) or (not game_data[i][4])):
                print 'Algorithm failed to notice a non-offensive play in ' + away_team + ' @ ' + home_team
                st = 1
            # Add the relevant information (time remaining, down, distance, field position, score differential, runpass) to the relevant list. Print an error if somehow 
            # I did something to make homeaway anything other than a 1 or a 0.
            if not st:
                if homeaway == 0:
...
def main():
    ...
    con = MySQLdb.connect('localhost', 'root', rootpass, 'NFL_Offensive_Plays_2015')
    print 'connection started'
    # team_number is a dict associating numbers 1-32 with teams. If I ever apply this to future seasons, I need to change 'St. Louis Rams' to 'Los Angeles Rams'.
    ...
    for game in games:
        ...
        # Here, I add the week number to the play info for each game. This will be useful later, when analyzing the data.
        ...
        # The below console message is useful if something goes wrong in this step: this way, we know for which games it worked and for which game it failed.
        print 'plays added for ' + game[2] + ' @ ' + game[1]
    # Next, I need to export play data to a SQL database.
    with con:
        cur = con.cursor()
